...
def main():
    ...
    parser.add_argument('url', help='Ссылка для сокращения или сокращенная ссылка')
    args = parser.parse_args()

    if is_shorten_link(vk_token, args.url):
        try:
            print(f'Количество переходов: {count_clicks(vk_token, args.url)}')
        ...
        try:
            print(f'Сокращенная ссылка: {shorten_link(vk_token, args.url)}')
        except ValueError as error:
            print(f'Ошибка: {error}')
        except requests.exceptions.HTTPError as error:
            print(f'Ошибка: {error}')
